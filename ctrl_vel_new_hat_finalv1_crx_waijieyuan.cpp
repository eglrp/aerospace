	...
	/*测距部分代码开始*/
		...
		if(!cap1.isOpened())
		{
			...
			*Iplimage_1_GRAY = IplImage(frame1_gray);
			...
			for (int m = 0; m <pcvSeqFaces0->total; m++)
			{
				...
				for (int i = 0; i < contours0.size(); i++)   //size()or size
				{
					if (contourArea(contours0[0], true) > max_area )
					{
						...
						max_index = i;   //max_index没有用到？？

                        minEnclosingCircle(Mat(contours0[i]),center0,radius0);//寻找最小外接圆
                        //circle(frame0,center0,radius0,Scalar(255,0,0),1,8);
					}
				}

				if (max_area > T)
				{
				    // 这里把昨天刘传奇写的框框的中心点改为了轮廓外接圆的圆心, added by crx
					rect_area_0[index0][0] = center0.x + r->x;
					rect_area_0[index0][1] = center0.y + r->y;
					rect_area_0[index0][2] = r->width;
					rect_area_0[index0][3] = r->height;
					rect_area_0[index0][4] = max_area;
					rect_area_0[index0][5] = radius0;
					index0 ++;
				}
			...
			for (int m = 0; m <pcvSeqFaces1->total; m++)
			{
				...
				for (int i = 0; i < contours1.size(); i++)   //size()or size
				{
					if (contourArea(contours1[0], true) > max_area)
					{
						...
						max_index = i;
                        minEnclosingCircle(Mat(contours1[i]),center1,radius1);//寻找最小外接圆
                        //circle(frame0,center0,radius0,Scalar(255,0,0),1,8);
				...
				if (max_area > T)
				{
					rect_area_1[index0][0] = center0.x + r->x;
					rect_area_1[index0][1] = center1.y + r->y;
					rect_area_1[index0][2] = r->width;
					rect_area_1[index0][3] = r->height;
					rect_area_1[index0][4] = max_area;
					rect_area_1[index0][5] = radius1;
					index0++;
				}
			...
			//修改 by crx
            int center_0_x = rect_area_0[index_max_0][0];
			int center_0_y = rect_area_0[index_max_0][1];
			...
			//修改 by crx
			int center_1_x = rect_area_1[index_max_1][0];
			int center_1_y = rect_area_1[index_max_1][1];
		...
		X_coor=(center_0_x-Cx)*Z_coor/fx;
		Y_coor=(center_0_y-Cy)*Z_coor/fy;
